    ...
    <div className="card-clean rounded-xl p-4 space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-900">تقارير منتصف الليل (00:00 Summaries)</h3>
            <p className="text-[11px] text-slate-500">
              ملخصات تنفيذية شاملة بالحركات المشبوهة وساعات العمل ومردودية العمال
            </p>
          </div>
        </div>
        <div className="flex items-center gap-1.5 text-xs text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded border border-emerald-200 font-semibold">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {reports.map((rep) => (
          <div
            key={rep.id}
            className="bg-slate-50/70 rounded-lg p-3.5 border border-slate-200 flex flex-col justify-between space-y-3"
          >
            <div className="flex items-center justify-between border-b border-slate-200 pb-2">
              <div className="flex items-center gap-2">
                <Calendar className="w-3.5 h-3.5 text-slate-500" />
                <span className="text-xs font-mono font-bold text-slate-800" dir="ltr">{rep.summary_date}</span>
                <span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-white text-slate-700 border border-slate-200 shadow-xs">
            ...
            <div className="text-xs text-slate-700 whitespace-pre-line leading-relaxed font-sans bg-white p-3 rounded border border-slate-200">
